...
var transition = {
    ...
    fire: function(new_layout, isotopes_focus, callback){
        ...
        // Unhighlight current nav
        if (display.current_layout != display.next_layout){
            display.highlightNav(display.current_layout, false);
        }

        // Fire the transition
        transition[display.current_layout][display.next_layout](function(){

            // Highlight the new nav
            display.highlightNav(display.next_layout, true);
            ...
            if (!display.loading && Modernizr.history){
                var focus = display.isotopes_focus;
                if (focus == null){ focus = display.next_isotopes_focus; }
                var title = "Nuclides.org - " + display.layouts[display.current_layout].title();
                var state = { layout: display.current_layout, focus: focus };
                var uri   = "?" + display.current_layout + (display.isotopes_focus != null ? "#" + display.isotopes_focus : "");
    ...
    moveCommonObjects: function(target, fade_in_delay, reset_element_focus){
        ...
        d3.timer(function(){
            (function(target, reset_element_focus){
                d3.select("#commons").style("display", "none");
                ...
                d3.select("#commons").style("display", null);
    ...
    setup: {
        elements: function(callback){
            ...
            transition.moveCommonObjects('elements', 0);
            callback();
        }
    },
    ...
    elements: {
        ...
        nuclides: function(callback){
            ...
            transition.moveCommonObjects('nuclides', 8000, true);

            // Hide element labels
            d3.selectAll("text.element_display").style("opacity", 0);

            // Fade out all elements and nuclides for repositioning
            d3.selectAll("g.element_display").transition()
                .duration(750 * display.transition_speed).style("opacity", 0);
            d3.selectAll("g.nuclide_display").transition()
                .duration(750 * display.transition_speed).style("opacity", 0);

            // Reposition/scale all elements and nuclides to small table position
            d3.timer(function(){
                // Remove from flow
                d3.selectAll("g.element_display").style("display", "none");
                d3.selectAll("g.nuclide_display").style("display", "none");
                // Reposition
                d3.selectAll("g.element_display")
                    .attr("transform", function(d){
                        var x = display.layouts.nuclides.getElementX(d);
                        var y = display.layouts.nuclides.getElementY(d);
                        return "translate("+x+","+y+")";
                    });
                d3.selectAll("g.nuclide_display")
                    .attr("transform", function(d){
            ...
            // Fade back in all elements and nuclides
            d3.selectAll("g.element_display").transition()
                .delay(1250 * display.transition_speed).duration(750 * display.transition_speed)
                .style("opacity", 1);
            d3.selectAll("g.nuclide_display").transition()
                .delay(1250 * display.transition_speed).duration(750 * display.transition_speed)
                .style("opacity", 1);
            ...
            // Loop through elements to reposition/scale nuclides to chart positions
            d3.timer(function(){
                for (var e in matter.elements){
                    ...
                    d3.selectAll("g.nuclide_display.e" + element.protons).transition()
                        .delay((50 * element.protons) * display.transition_speed)
            ...
            // Done
            d3.timer(function(){
                // ensure all nuclides are showing
                d3.selectAll("g.nuclide_display").style("opacity", 1);
                callback();
                return true;
            }, 9000 * display.transition_speed);
        ...
        isotopes: function(callback){
            ...
            transition.moveCommonObjects('isotopes', 4000);

            // Hide element labels
            d3.selectAll("text.element_display").style("opacity", 0);

            // Hide all but focused nuclides
            d3.selectAll("g.nuclide_display").filter("*:not(.e" + display.next_isotopes_focus + ")").transition()
                .duration(500 * display.transition_speed)
                .style("opacity", 0);
            // Ensure focused nuclides are shown
            d3.selectAll("g.nuclide_display.e" + display.next_isotopes_focus).style("opacity", 1);
            ...
            d3.timer(function(){
                // Remove from flow
                d3.selectAll("g.nuclide_display").filter("*:not(.e" + display.next_isotopes_focus + ")").style("display", "none");
                // Reposition
                d3.selectAll("g.nuclide_display").filter("*:not(.e" + display.next_isotopes_focus + ")")
                    .attr("transform", function(d){
                ...
                // Scale
                d3.selectAll("rect.nuclide_display").filter("*:not(.e" + display.next_isotopes_focus + ")")
                ...
                // Restore to flow
                d3.selectAll("g.nuclide_display").filter("*:not(.e" + display.next_isotopes_focus + ")").style("display", null);
                return true;
            }, 600 * display.transition_speed);
    ...
    nuclides: {
        ...
        elements: function(callback){
            ...
            // Done
            d3.timer(function(){ callback(); return true; }, 12000 * display.transition_speed);
    ...
    isotopes: {
        ...
        elements: function(callback){
           ...
           // Hide nuclide scale
            ...
            d3.timer(function(){
                d3.select("#floating_element_highlightbox").attr("display", "none");
                return true;
            }, 3000 * display.transition_speed);
        ...
        nuclides: function(callback){
            ...
            // Translate elements into position
            d3.selectAll("g.element_display").transition()
                .delay(9500 * display.transition_speed)
                .duration(1000 * display.transition_speed)
                .attr("transform", function(d){
                    var x = display.layouts.nuclides.getElementX(d);
                    var y = display.layouts.nuclides.getElementY(d);
            ...
            d3.timer(function(){
                ...
                callback();
                return true;
            }, 10500 * display.transition_speed);
        ...
        isotopes: function(callback){
            ...
            // Done
            d3.timer(function(){ callback(); return true; }, 6500 * display.transition_speed);
        },

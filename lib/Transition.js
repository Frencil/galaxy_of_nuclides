...
var transition = {
    ...
    fire: function(new_layout, element_detail_focus){
        ...
        if (typeof element_detail_focus !== undefined && new_layout == 'element_detail'){
            transition.set_element_focus('next', element_detail_focus);
        }
    ...
    set_element_focus: function(slot, protons){
        if (slot == 'current'){
        ...
        if (display[slot] != null){
            d3.select("rect.element_display.e" + display[slot])
                .attr("class", "element_display e" + display[slot]);
        ...
        if (protons != null){
            d3.select("rect.element_display.e" + protons)
                .attr("class", "element_display e" + protons + " focused");
            if (display.current_layout == 'chart_of_nuclides'){
    ...
    swap_element_focus: function(){
        var new_current_focus = display.next_element_detail_focus;
        transition.set_element_focus('next', null);
        transition.set_element_focus('current', new_current_focus);
    },

    // Set image URL
    set_image: function(protons, transition_duration){
        ...
        if (typeof transition_duration == 'undefined'){
            ...
            var dark = transition_duration / 5;
            var ease = dark * 2;
    ...
    set_nuclide_hitboxes: function(protons){
            ...
            for (var n = 0; n < matter.elements[protons].neutron_spread; n++){
                ...
                d3.select("#nuclide_" + n + "_hitbox")
                    ...
                    .on("mouseover", function(){
                        ...
                        d3.select(nuclide_rect_class)
                            .attr("class", "nuclide_display e" + protons + " n" + neutrons + " focused");
                    })
                    .on("mouseout", function(){
                        ...
                        var nuclide_rect_class = "rect.nuclide_display.e" + protons + ".n" + neutrons;
                        d3.select(nuclide_rect_class)
                            .attr("class", "nuclide_display e" + protons + " n" + neutrons);
                    });
            }
        }
    },
    ...
    moveInfoBoxes: function(target, fade_in_delay, reset_element_focus){
        ...
        setTimeout(function(){
            (function(target, reset_element_focus){
                ...
                d3.select("#image_border")
                    .attr("x", display.layouts[target].image.x * display.scale)
                    .attr("y", display.layouts[target].image.y * display.scale)
                    .attr("width", display.layouts[target].image.w * display.scale)
                    .attr("height", display.layouts[target].image.h * display.scale)
                if (typeof reset_element_focus !== undefined && reset_element_focus){
                    transition.set_element_focus('current', null);
                    transition.set_element_focus('next', null);
    ...
    periodic_table: {
        ...
        element_detail: function(callback){
            ...
            if (display.next_element_detail_focus == null){
                ...
                transition.set_element_focus('next', rand);
            }
            ...
            // Done
            setTimeout(function(){
                transition.set_nuclide_hitboxes(display.next_element_detail_focus);
                transition.swap_element_focus();
                callback();
            }, 6000);
    ...
    element_detail: {
        ...
        periodic_table: function(callback){
            ...
            // Reposition info boxes
            transition.moveInfoBoxes('periodic_table', 5500, true);

            // Disable nuclide hitboxes
            transition.set_nuclide_hitboxes(null);
            ...
            setTimeout(function(){ callback(); }, 5500);
        ...
        element_detail: function(callback){
            ...
            // Switch the image
            setTimeout(function(){
                transition.set_image(display.next_element_detail_focus, 1000);
            }, 2500);

...
var display = {
    ...
    layouts: { periodic_table:    { data: { x: 6, y: 28, w: 180, h: 100 },
                                    ...
                                    setHitboxes: function(){
                                        var w = display.scale * (display.layouts.periodic_table.element.w
                                                               + display.layouts.periodic_table.element.m);
               ...
               chart_of_nuclides: { data: { x: 5, y: 7, w: 180, h: 120 },
                                    ...
                                    drawAxes: function(){
                                        ...
                                        var end_y     = 6;
                                        var origin_x  = display.layouts.chart_of_nuclides.data.x;
                                        var origin_y  = display.regions.stage.h - display.layouts.chart_of_nuclides.data.y;
                                        var tick_step = 10 * (display.layouts.chart_of_nuclides.nuclide.w + display.layouts.chart_of_nuclides.nuclide.m);
                                        ...
                                        for (var t = 0; t < end_x - start_x; t += tick_step){
                                            if (t < (start_y - end_y)){
                                                var tick_y = (display.layouts.chart_of_nuclides.data.h - t) + display.layouts.chart_of_nuclides.data.y;
                                                axes.append("path")
                                    ...
                                    setHitboxes: function(){
                                        var ew = display.scale * (display.layouts.chart_of_nuclides.element.w
                                                                + display.layouts.chart_of_nuclides.element.m);
                                        d3.selectAll(".hitbox.element")
                                            .attr("display", null)
                                        ...
                                        var nw = display.scale * (display.layouts.chart_of_nuclides.nuclide.w
                                                                + display.layouts.chart_of_nuclides.nuclide.m);
               ...
               element_detail:    { data: { x: 136, y: 4, w: 52, h: 52 },
                                    ...
                                    setInfo: function(){
                                        d3.selectAll(".blurb").remove();
                                        if (display.next_element_detail_focus == 0){
                                        ...
                                        var blurb = d3.select("#info").append("text")
                                            ...
                                            .attr("class", "info blurb")
                                            .attr("font-size", line_height * 0.85 + "px");
                                        var content = matter.elements[display.next_element_detail_focus].info;
                                        display.printTextBlock(blurb, content, origin_x, origin_y + title_height + line_height, line_height);
                                    },
                                    setHitboxes: function(){
                                        var ew = display.scale * (display.layouts.element_detail.element.w
                                                                + display.layouts.element_detail.element.m);
                                        d3.selectAll(".hitbox.element")
                                            .attr("display", null)
                                            .attr("width", ew).attr("height", ew)
                                        ...
                                        var nw = display.scale * (display.layouts.element_detail.nuclide.w
                                                                + display.layouts.element_detail.nuclide.m);
                                        d3.selectAll(".hitbox.nuclide.e" + display.element_detail_focus)
                                            .attr("display", null)
                                            .attr("width", nw).attr("height", nw)
                                            .attr("transform", function(d){
    ...
    highlightElement: function(protons, bool){
        if (display.in_transition){ return; }
        var selector = "rect.element_display.e" + protons;
        if (bool){  // Highlight
            ...
            display.setImage(protons);
        }  else  {  // Un-Highlight
            d3.select(selector).attr("filter", "none");
            if (display.current_layout == 'chart_of_nuclides'){
                d3.select("#element_" + protons + "_highlightbox").attr("display", "none");
            }
            if (display.current_layout == 'element_detail'){
                display.setImage(display.element_detail_focus);
            }
        }
    },

    highlightNuclide: function(protons, neutrons, bool){
        if (display.in_transition){ return; }
        var selector = "rect.nuclide_display.e" + protons + ".n" + neutrons;
        if (bool){  // Highlight
            d3.select(selector).attr("filter", "url(#highlight-glow)");
        }  else  {  // Un-Highlight
            d3.select(selector).attr("filter", "none");
        ...
        // Cascade
        display.highlightElement(protons, bool);
    },
                ...
                if (typeof element != "undefined"){
                    (function(node, element){
                        node.on("mouseover", function(){
                            ...
                            transition.fire("element_detail", element);
                        });
                    })(node, element);
                } else if (typeof nuclide != "undefined"){
                    (function(node, nuclide){
                        node.on("mouseover", function(){
                            display.highlightNuclide(nuclide.p, nuclide.n, true);
                        }).on("mouseout", function(){
                            display.highlightNuclide(nuclide.p, nuclide.n, false);
                        }).on("click", function(){
                            transition.fire("element_detail", nuclide.p);
                        });
                    })(node, nuclide);
                }
            }

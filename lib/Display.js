...
var display = {
    ...
    hidePeriodicTable: function(duration){
        if (typeof duration != 'number'){ var duration = 0; }
        display.fadeOut(d3.selectAll("g.element_display"), duration);
    },

    showPeriodicTable: function(override_settings){
        ...
        var settings = {
            ...
            nuclide: { w: 0, m: 0 },
            show_labels: false,
            transition: { duration: 0, delay: 0, stagger_delay: 0 }
        }
        ...
        // Position and show element groups
        d3.selectAll("g.element_display").style("display", null);
        ...
        // Size and show element rects
        d3.selectAll("rect.element_display").transition()
            .delay(settings.transition.delay * display.transition_speed )
            .duration(settings.transition.duration * display.transition_speed)
            .attr("width", settings.element.w * display.scale)
            .attr("height", settings.element.w * display.scale)
            .style("opacity", 1);

        // Position and show element labels
        if (settings.show_labels){
            display.fadeIn(d3.selectAll("text.element_label"), settings.transition.duration);
            var label_unit = settings.element.w / 10;
            d3.selectAll("tspan.element_symbol").transition()
                .delay(settings.transition.delay * display.transition_speed)
                .duration(settings.transition.duration * display.transition_speed)
                ...
                .style("opacity", 1);
            d3.selectAll("tspan.element_number").transition()
                .delay(settings.transition.delay * display.transition_speed)
                .duration(settings.transition.duration * display.transition_speed)
                ...
                .style("opacity", 1);
            d3.selectAll("tspan.element_symbol.l3")
                .attr("textLength", (7 * label_unit * display.scale) + "px")
                .attr("lengthAdjust", "spacingAndGlyphs");
        } else {
            display.fadeOut(d3.selectAll("text.element_label"), settings.transition.duration);
        }
    },

    hideChartOfNuclides: function(){
        if (typeof duration != 'number'){ var duration = 0; }
        display.fadeOut(d3.selectAll("g.nuclide_display"), duration);
    },

    showChartOfNuclides: function(override_settings){
        ...
        // Position and show nuclide groups
        d3.selectAll("g.nuclide_display").style("display", null);
        d3.selectAll("g.nuclide_display").transition()
            .delay(settings.transition.delay * display.transition_speed)
            .duration(settings.transition.duration * display.transition_speed)
            .attr("transform", function(d){
                var coords = display.chart_of_nuclides.getNuclideCoords(d, settings)
                return "translate(" + coords[0] + "," + coords[1] + ")";
            }).style("opacity", 1);

        // Size and show nuclide rects
        d3.selectAll("rect.nuclide_display").transition()
            .delay(settings.transition.delay * display.transition_speed )
            .duration(settings.transition.duration * display.transition_speed)
            .attr("width", settings.nuclide.w * display.scale)
            .attr("height", settings.nuclide.w * display.scale)
            .style("opacity", 1);

        // Hide nuclide labels in full chart view
        display.fadeOut(d3.selectAll("text.nuclide_label"), settings.transition.duration);

    },

    showCaptions: function(duration, delay, stagger_delay){
        ...
        for (var c = 0; c < questions.next.captions.length; c++){
            ...
            var text_node = d3.select("#captions").append("text")
                ...
                .style("opacity", 0);
            display.printTextBlock(text_node, caption.copy, caption.x * display.scale, caption.y * display.scale, caption.line_height * display.scale);
    ...
    showComponents: function(duration, delay, stagger_delay){
    ...
    showQuestions: function(question_list){
        d3.select("#qlist").selectAll("tspan").remove();
        for (var q in question_list){
            (function(q, question_list){
                var title = question_list[q];
                var id = questions.titleToID(title);
                var text = [];
                ...
                while (title.length > 0){
                    if (title.length <= max_line_length){
                        ...
                        var idx = title.slice(0,max_line_length).lastIndexOf(" ");
                        text.push(title.slice(0, idx).trim());
                ...
                for (var t in text){
                    ...
                    var tspan = d3.select("#qlist").append("tspan")
                        .attr("class", "qhref").attr("x", 0).attr("dy", dy)
    ...
    highlightElement: function(element, bool){
        ...
        if (bool){  // Highlight
            ...
            if (display.current_layout == 'isotopes'){
                display.setThumbnail(display.isotopes_focus);
            }
        }
    },
    ...
    printTextBlock: function(selector, raw_text, origin_x, origin_y, line_height){
        ...
        var dy_adjustments = { 'sup': -1 * display.scale,
                               ...
                               'sml': 0 };
        var n = 0, t = 0;
        tspans.push({ text: '', class: [], mouse: null, title: null,
            ...
            case '[em3]':
                ...
                if (emphasis == null){
                    ...
                    if (!tspans[t].text.length){
                        ...
                        tspans.push({ text: '', class: [emphasis, position], title: null,
...
display.regions.questions.draw = function(){
    ...
    d3.select("#questions").append("text").append("tspan")
        .attr("class", "qhref")
        .attr("x", 0).attr("y", 3 * display.scale)
        .style("font-size", (3 * display.scale) + "px")
        .style("font-weight", "bold")
        .text("Help - What is Nuclides.org?")
        .on("click", function(){
            questions.call("what_is_nuclides_org", function(){});
        });
    ...
    d3.select("#dataset").selectAll(".element_display")
        ...
        .each(function(d, i) {
            ...
            var label = d3.select("#" + display_id)
                .append("text").attr("class", "element_label")
                .attr("id", "element_" + d.protons + "_label")
                .attr("x", 0).attr("y", 0).style("display", "none");
            var tspan = label.append("tspan")
                .attr("class", "element_symbol e" + d.protons + " l" + d.symbol.length)
                .attr("x", 0).attr("y", 0).style("font-size", "0px")
                .text(d.symbol);
            label.append("tspan")
                .attr("class", "element_number e" + d.protons + " l" + d.symbol.length)
                .attr("x", 0).attr("y", 0).style("font-size", "0px")
                .attr("text-anchor", "end")
                .text(d.protons);
        });
    ...
    d3.select("#dataset").selectAll(".nuclide_display")
        ...
        .each(function(d, i){
            ...
            var label = d3.select("#" + display_id)
                ...
                .attr("class", function(d){ return "nuclide_label e" + d.protons + " n" + d.neutrons; })
                .attr("fill", function(d){ return palette.nuclideLabel(d); })
                .attr("x", 0)
                .attr("y", 0)
                .style("opacity",0);
            var label_unit = 1; //(display.layouts.isotopes.nuclide.w * display.scale) / 10;
            var tspan = label.append("tspan")

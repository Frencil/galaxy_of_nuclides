...
var display = {
    ...
    showPeriodicTable: function(duration, delay, stagger_delay){
        // Sanity check
        ...
        for (var e = 0; e < matter.elements; e++){
            var element = matter.elements[e];
            d3.select("#element_" + element.protons + "_display").transition()
                .delay((delay + stagger_delay * element.protons) * display.transition_speed)
                .duration(duration * display.transition_speed)
                .attr("transform", function(d){
                    var coords = questions.next.periodic_table.getElementCoords(d)
                    return "translate(" + coords[0] + "," + coords[1] + ")";
                }).style("opacity", 1);
            d3.selectAll("rect.element_display.e" + element.protons).transition()
                .delay((delay + stagger_delay * element.protons) * display.transition_speed)
                .duration(duration * display.transition_speed)
                .attr("width", questions.next.periodic_table.element.w * display.scale)
                .attr("height", questions.next.periodic_table.element.w * display.scale);
        }
    },

...
var display = {
    ...
    setElapsedTimeExp: function(value){
        ...
        var numerical = t.repNumerical();
        d3.select("#key_elapsed_time_numerical").text(numerical);
        ...
        if (caption.length){
            display.printTextBlock(d3.select("#key_elapsed_time_caption"), caption,
                                   2.5 * display.scale, 11 * display.scale, 2.5 * display.scale);
        }
    },
    ...
    regions: {
        ...
        nav: {
            x: 0, y: 28, w: 44, h: 108,
            regions: {
                elements: { x: 0, y: 0, w: 44, h: 36, title: "Show the Periodic Table of Elements" },
                nuclides: { x: 0, y: 36, w: 44, h: 36, title: "Show the Chart of Nuclides (all Isotopes of all Elements)" },
                isotopes: { x: 0, y: 72, w: 44, h: 36, title: "Show the Isotopes for a single Element" },
    ...
    setZoomNuclide: function(nuclide){
        if (nuclide == null || (display.current_layout != 'nuclides' && display.current_layout != 'isotopes')){
            ...
            d3.select("#zoom_nuclide_info_halflife").text("Half-Life: " + nuclide.halflife.repNumerical(false));
        ...
        while (n < nodes.length){
            ...
            case '[em3]':
                ...
                if (tspans[t].class == node_class){
                    tspans.push({ text: '', class: null, mouse: null, x: null, y: null, dy: null });
                    t++;
                } else {
                    if (tspans[t].text.length == 0){
                        tspans[t].class = node_class;
                    } else {
                        tspans.push({ text: '', class: node_class, mouse: null, x: null, y: null, dy: null });
                        t++;
                    }
                }
                break;
...
display.regions.key.init = function(){
    ...
    display.time_slider.mouse_events = d3.behavior.drag()
        ...
        .on("drag", function(d){
            display.time_slider.highlightSlider(true);
            display.time_slider.x = display.time_slider.x_origin;
            display.time_slider.y = Math.min(Math.max(d3.event.y, display.time_slider.y_min), display.time_slider.y_max);
            var transform = "translate(" + display.time_slider.x + ", " + display.time_slider.y + ")";
        ...
        .on("dragend", function(d){
            display.time_slider.highlightSlider(false);
            var transform = "translate(" + display.time_slider.x + ", " + display.time_slider.y + ")";
...
display.regions.key.regions.slider.draw = function(){
    ...
    var nscale = d3.select("#slider").append("g")
        .attr("id", "key_nuclide_scale").style("opacity", 0);
    nscale.append("title")
    ...
    var handle_g = nscale.append("g")
        ...
        .attr("class", "handle")
        .attr("transform", "translate(" + display.time_slider.x + "," + display.time_slider.y + ")");
    ...
    handle_g.append("path")
        .attr("d",pathString([2, 1, display.time_slider.w - 2, 1]))
        .attr("stroke","rgb(127,127,127)").style("stroke-width", "1");
    handle_g.append("path")
        .attr("d",pathString([2, 2, display.time_slider.w - 2, 2]))
        .attr("stroke","rgb(127,127,127)").style("stroke-width", "1");

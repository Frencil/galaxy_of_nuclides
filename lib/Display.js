...
var display = {
    ...
    showPeriodicTable: function(settings){

        // Todo: make this behave as an override on a per-attribute basis
        if (typeof settings == 'undefined'){
            var settings = questions.next.periodic_table;
        }
        var t = settings.transition;

        if (typeof t.duration != 'number'){ t.duration = 0; }
        if (typeof t.delay != 'number'){ t.delay = 0; }
        
        // Override question-set transition parameters. Feels hacky!
        if (!questions.current.all_nuclides_shown){
            t.duration = 500;
            t.delay = t.stagger_delay = 0;
        }

        // Position and show element groups
        d3.selectAll("g.element_display").transition()
            .delay(t.delay * display.transition_speed)
            .duration(t.duration * display.transition_speed)
            .attr("transform", function(d){
                var coords = settings.getElementCoords(d)

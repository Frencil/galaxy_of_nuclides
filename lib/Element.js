function Element (protons, period, group) { 
  ...
  this.addNuclide = function(nuclide) {
    ...
    // global values
    window.elements.max_neutron_spread    = Math.max(this.max_neutrons - this.min_neutrons, window.elements.max_neutron_spread);
    window.elements.absolute_max_neutrons = Math.max(this.max_neutrons, window.elements.absolute_max_neutrons);
    window.elements.max_nuclides_per_element = Math.max(window.elements.max_nuclides_per_element, Object.keys(this.nuclides).length);
  }

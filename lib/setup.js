...
var time_slider = {
    mouse_events: d3.behavior.drag()
        .origin(function(d) { return d; })
        .on("dragstart", function(d){
            d3.select("#time_slider_handle_hitbox").style("display", "none");
            time_slider.highlightSlider(true);
        })
        .on("drag", function(d){
            time_slider.highlightSlider(true);
            time_slider.x = time_slider.x_origin;
            time_slider.y = Math.min(Math.max(d3.event.y, time_slider.y_min), time_slider.y_max);
            var transform = "translate(" + time_slider.x + ", " + time_slider.y + ")";
            d3.select("#time_slider_handle").attr("transform", transform);
            d3.select("#time_slider_shadow").attr("height", time_slider.y - time_slider.y_min);
            var new_elapsed_time_exp = null;
            if (time_slider.y > time_slider.y_min){
                new_elapsed_time_exp =
                    matter.min_halflife_exp
                    + (matter.max_halflife_exp - matter.min_halflife_exp)
                    * (time_slider.y - time_slider.y_min) / (time_slider.y_max - time_slider.y_min);
            }
            display.setElapsedTimeExp(new_elapsed_time_exp);
        })
        .on("dragend", function(d){
            var transform = "translate(" + time_slider.x + ", " + time_slider.y + ")";
            d3.select("#time_slider_handle_hitbox")
                .attr("transform", transform)
                .style("display", null);
        }),
    highlightSlider: function(bool){
        if (bool){
...
function setDisplayVars(){
    ...
    // Build time slider object
    time_slider.y_margin = 15 * display.scale;
    time_slider.slider_height = 3 * display.scale;
    time_slider.slider_width = 14 * display.scale;
    time_slider.x_origin = ((display.regions.time.regions.slider.w / 2) * display.scale) - (time_slider.slider_width / 2);
    time_slider.y_min = time_slider.y_margin;
    time_slider.y_max = (display.regions.time.regions.slider.h * display.scale) - (time_slider.y_margin + time_slider.slider_height);
    time_slider.x = time_slider.x_origin;
    time_slider.y = time_slider.y_min;
...
function renderApplication(callback){
    ...
    nscale.append("rect")
        ...
        .attr("x", ((w / 2) - 3) * display.scale).attr("y", time_slider.y_margin)
        .attr("width", 6 * display.scale).attr("height", (h * display.scale) - (2 * time_slider.y_margin))
    ...
    var shadow = nscale.append("rect")
        ...
        .attr("x", ((w / 2) - 3) * display.scale).attr("y", time_slider.y).attr("width", 6 * display.scale).attr("height", 0)
        .attr("fill", "rgba(0,0,0,0.75)");
    var handle_g = nscale.append("g")
        .attr("id", "time_slider_handle")
        .attr("class", "handle")
        .attr("transform", "translate(" + time_slider.x + "," + time_slider.y + ")");
    ...
    hitbox_g.append("rect")
        .attr("class", "hitbox")
        .attr("width", 14 * display.scale).attr("height", 3 * display.scale)
        .call(time_slider.mouse_events)
        .on("mouseover", function(){ time_slider.highlightSlider(true); })
        .on("mouseout", function(){ time_slider.highlightSlider(false); });

...
var display = {
    ...
    regions: { stage:      { x: 44, y: 28, w: 192, h: 132 },
               ...
               nav: { x: 0, y: 100, w: 44, h: 36,
                      regions: { periodic_table:    { x: 0, y: 28, w: 44, h: 36 },
                                 chart_of_nuclides: { x: 0, y: 64, w: 44, h: 36 },
                                 element_detail:    { x: 0, y: 100, w: 44, h: 36 }
                               }
                    }
             },
...
function drawGalaxy(callback){
    ...
    var drawRegions = function(regions){
        for (var region in regions){
            ...
            if (r.regions !== undefined){
                drawRegions(r.regions);
            }
        }
    };
    drawRegions(display.regions);      
    ...
    d3.xml("images/svg/nav_brackets.svg", "image/svg+xml", function(xml) {
        var importedNode = document.importNode(xml.documentElement, true);
        d3.select("#nav")

...
function drawGalaxy(callback){
            ...
            d3.select("#"+this.id).selectAll(".nuclide")
                ...
                .each(function(d, i){
                    ...
                    var tspan = label.append("tspan")
                        ...
                        .style("font-size", 4 * label_unit * display.scale + "px")
                        .text(element.symbol);
                    if (element.symbol.length > 2){
                        tspan.attr("textLength", 4 * label_unit * display.scale + "px").attr("lengthAdjust", "spacingAndGlyphs");
                    }
                    var tspan2 = label.append("tspan")
                        ...
                        .text(d.neutrons);
                    if (d.neutrons > 99){
                        tspan2.attr("textLength", 3.1 * label_unit * display.scale + "px").attr("lengthAdjust", "spacingAndGlyphs");
                    }
                });
        });
    ...
    d3.selectAll(".element_hitbox_group")
        ...
        .on("mouseover", function(d){
            display.highlightElement(d.protons, true);
        })
        .on("mouseout", function(d){
            display.highlightElement(d.protons, false);
        })
        .on("click", function(d){
    ...
    d3.selectAll(".element_hitbox_group")
        ...
        .each(function(d, i) {
            d3.select("#"+this.id).selectAll(".hitbox.nuclide")
                ...
                .attr("display", "none")
                .on("mouseover", function(d){
                    display.highlightNuclide(d.protons, d.neutrons, true);
                })
                .on("mouseout", function(d){
                    display.highlightNuclide(d.protons, d.neutrons, false);
                })
                .on("click", function(d){
                    transition.fire("element_detail", d.protons);
                });
    ...
    /*
    setTimeout(function(){
        transition.fire("element_detail", 0);

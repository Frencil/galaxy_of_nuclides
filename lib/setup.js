...
function drawGalaxy(callback){
    ...
    var dataset = d3.select("#dataset").selectAll(".element_shell")
        ...
        .each(function(d, i) {
            ...
            var label = d3.select("#" + display_id)
                ...
                .attr("x", 0)
                .attr("y", 0);
            var label_unit = (display.layouts.periodic_table.element.w / 10);
            var tspan = label.append("tspan")
                .attr("x", 1 * label_unit * display.scale)
                .attr("y", 8 * label_unit * display.scale)
                .style("font-size", 5 * label_unit * display.scale)
                .text(d.symbol);
            if (d.symbol.length > 2){
                tspan.attr("textLength", 7 * label_unit * display.scale).attr("lengthAdjust", "spacingAndGlyphs");
            }
            label.append("tspan")
                .attr("x", 9 * label_unit * display.scale)
                .attr("y", 3 * label_unit * display.scale)
                .attr("text-anchor", "end")
                .style("font-size", 2.5 * label_unit * display.scale)
                .text(d.protons);
        });
            ...
            d3.select("#"+this.id).selectAll(".nuclide")
                ...
                .each(function(d, i){
                    ...
                    var label = d3.select("#" + display_id)
                        ...
                        .attr("y", 0)
                        .style("opacity",0);
                    var label_unit = (display.layouts.element_detail.nuclide.w / 10);
                    var tspan = label.append("tspan")
                        .attr("x", 4.1 * label_unit * display.scale)
                        .attr("y", 7 * label_unit * display.scale)
                        .style("font-size", 4 * label_unit * display.scale)
                        .text(element.symbol);
                    if (element.symbol.length > 2){
                        tspan.attr("textLength", 4 * label_unit * display.scale).attr("lengthAdjust", "spacingAndGlyphs");
                    }
                    var tspan2 = label.append("tspan")
                        .attr("x", 3.8 * label_unit * display.scale)
                        .attr("y", 4 * label_unit * display.scale)
                        .attr("text-anchor", "end")
                        .style("font-size", 2.5 * label_unit * display.scale)
                        .text(d.neutrons);
                    if (d.neutrons > 99){
                        tspan2.attr("textLength", 3.1 * label_unit * display.scale).attr("lengthAdjust", "spacingAndGlyphs");
                    }
                });
        });
    ...
    /*
    setTimeout(function(){
        transition.fire("element_detail", 1);
    }, matter.elements.length * 20 + 900);
    */

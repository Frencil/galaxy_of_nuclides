    ...
    translate: {
        dataset: {
            ...
            chart_of_nuclides: function(){
                ...
                var y_margin = display.layouts.chart_of_nuclides.data.y * display.scale;
                return "translate("+x_margin+","+y_margin+")";
            },
            reset: function(){
...
function setDisplayVars(){
    ...
    display.layouts.periodic_table.nuclide.w = periodic_table_base_nuclide_width * .8;
    display.layouts.periodic_table.nuclide.m = periodic_table_base_nuclide_width * .2;
}
...
function drawGalaxy(callback){
    ...
    // Add groups for each of the five top-level display regions
    for (var region in display.regions){
        var translate = "translate(" + display.regions[region].x * display.scale + ","
                                     + display.regions[region].y * display.scale + ")";
        d3.select("svg").append("g")
            .attr("id", region)
            .attr("transform", translate);
        // temporary background rect for easy inspection of empty / part-filled regions
        d3.select("#"+region).append("rect")
            .attr("x", 0)
            .attr("y", 0)
            .attr("width", display.regions[region].w * display.scale)
            .attr("height", display.regions[region].h * display.scale)
            .style("opacity", 0);
    }

    // Draw control elements (WIP)
    d3.select("#nav").append("g")
        .attr("id", "controls")
        .attr("class", "controls");
    d3.select("#controls")
        .append("text")

...
var display = {
    ...
    layouts: { periodic_table:    { data: { x: 6, y: 26, w: 180, h: 100 },
                                    ...
                                    getElementX: function(group, period){
                                        ...
                                        if (group > 18){
                                            return x + (group - 17) * base;
                                        } else if (group == 0) {
                                            return -100 * base;
                                        } else {
                                            return x + (group - 1) * base;
                                        }
                                    },
                                    getElementY: function(group, period){
                                        ...
                                        if (group > 18){
                                            return y + (period + 2) * base;
                                        } else if (group == 0) {
                                            return y;
                                        } else {
                                            return y + (period - 1) * base;
                                        }
                                    },
                                    getNuclideX: function(nuclide){
                                        ...
                                        var origin  = display.layouts.periodic_table.getElementX(element.group, element.period);
                                    ...
                                    getNuclideY: function(nuclide){
                                        var base = display.scale * (display.layouts.chart_of_nuclides.nuclide.w
                                                                  + display.layouts.chart_of_nuclides.nuclide.m);
                                        var element = matter.elements[nuclide.protons];
                                        var origin  = display.layouts.periodic_table.getElementY(element.group, element.period);
                                        return origin + display.scale * display.layouts.chart_of_nuclides.nuclide.m
    ...
    var dataset = d3.select("#dataset").selectAll(".element_shell")
        ...
        .each(function(d, i) {
            ...
            d3.select("#"+this.id)
                ...
                .attr("transform", function(d){ return transform; });
    ...
    dataset.append("g")
        ...
        .each(function(d, i) {
            d3.select("#"+this.id).selectAll(".nuclide")
                ...
                .enter().append("g")
                .attr("class", "nuclide_shell element_" + d.protons)
    ...
    periodic_table: {
        ...
        chart_of_nuclides: function(callback){
            ...
            info_transition.select("#info1")
                .attr("x", display.layouts.chart_of_nuclides.info1.x * display.scale)
                .attr("y", display.layouts.chart_of_nuclides.info1.y * display.scale)
                .attr("width", display.layouts.chart_of_nuclides.info1.w * display.scale)
                .attr("height", display.layouts.chart_of_nuclides.info1.h * display.scale)
            info_transition.select("#info2")
                .attr("x", display.layouts.chart_of_nuclides.info2.x * display.scale)
                .attr("y", display.layouts.chart_of_nuclides.info2.y * display.scale)
                .attr("width", display.layouts.chart_of_nuclides.info2.w * display.scale)
                .attr("height", display.layouts.chart_of_nuclides.info2.h * display.scale)
            ...
            for (var e in matter.elements){
                ...
                d3.selectAll(".nuclide_shell").filter(".element_" + element.protons).transition()
                    .delay(1100 + 16 * element.protons).duration(2000)
                    .attr("transform", transform.translate.nuclide.chart_of_nuclides);

function loadElements(callback) {
    window.matter = new Object();
    window.matter.min_halflife_exp = 0;
    window.matter.max_halflife_exp = 0;
    window.matter.max_neutron_spread    = 0;
    window.matter.absolute_max_neutrons = 0;
    window.matter.max_nuclides_per_element = 0;
    window.matter.elements = new Array();
...
function parseElements(data) {
    ...
    for (var i = 0; i < elements_csv_parsed.length; i++) {
        ...
        if (protons >= 0){
            ...
            window.matter.elements[protons] = element;
